...
async function activateTerminal(page: Page, worktreeId: string, tabId: string): Promise<void> {
  ...
  await ensureTerminalVisible(page)
  await waitForActiveTerminalManager(page, 30_000)
  await page.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"]`).click()
}
...
async function typeIntoTerminal(page: Page, tabId: string, marker: string): Promise<boolean> {
...
test('adopts runtime-owned agent and Setup PTYs on first mount', async ({
  ...
  expect(await typeIntoTerminal(orcaPage, agent!.tabId, agentMarker)).toBe(true)
  ...
  await clearTerminalPtyWriteLog(electronApp)
  expect(await typeIntoTerminal(orcaPage, setup!.tabId, setupMarker)).toBe(true)
  ...
  await expect(terminalAccessibility(orcaPage, agent!.tabId)).toContainText(
    `AGENT_INPUT:${agentPid}:${remountAgentLiveMarker}`
  )
  ...
  await expect(terminalAccessibility(orcaPage, agent!.tabId)).toContainText(
    `AGENT_INPUT:${agentPid}:${remountAgentAcceptedMarker}`
  )
  const remountAgentMarker = `AGENT_REMOUNT_${randomUUID().slice(0, 8)}`
  await clearTerminalPtyWriteLog(electronApp)
  expect(await typeIntoTerminal(orcaPage, agent!.tabId, remountAgentMarker)).toBe(true)
  await assertExactPtyReceivedMarker(electronApp, agent!.ptyId, remountAgentMarker)
  await expect(terminalAccessibility(orcaPage, agent!.tabId)).toContainText(
    `AGENT_INPUT:${agentPid}:${remountAgentMarker}`
  )
  await expect(terminalAccessibility(orcaPage, agent!.tabId)).not.toContainText(
    'Conversation interrupted'
  )
  ...
  await expect(terminalAccessibility(orcaPage, setup!.tabId)).toContainText(
    `SETUP_INPUT:${setupPid}:${remountSetupLiveMarker}`
  )
  const remountSetupMarker = `SETUP_REMOUNT_${randomUUID().slice(0, 8)}`
  await clearTerminalPtyWriteLog(electronApp)
  expect(await typeIntoTerminal(orcaPage, setup!.tabId, remountSetupMarker)).toBe(true)
  await assertExactPtyReceivedMarker(electronApp, setup!.ptyId, remountSetupMarker)
  await expect(
    terminalAccessibility(orcaPage, setup!.tabId),
    'Setup remount must remain bound to the original live process'
  ).toContainText(`SETUP_INPUT:${setupPid}:${remountSetupMarker}`)
